...
**Version:** 1.14 (rounds 1–9 adversarially reviewed and corrected; 1.10–1.14 record execution findings E-1…E-20 from implementing Phases 0–5 and the installation surface)
...
- **1.14** — Installation-surface findings, added after Phases 0–5 (E-18…E-20). **E-18 (additive §5.1 change):** `ToolDescriptor` gains one optional field, `meta: Option<serde_json::Value>` (`#[serde(default, skip_serializing_if = "Option::is_none")]`), copied verbatim into each `tools/list` entry as `_meta`. It carries `anthropic/maxResultSizeChars`, which raises Claude Code's per-tool result ceiling for the three tools whose **text** output can exceed the ~10k-token persist-to-disk threshold: `node.list_types`, `graph.list_nodes`, and `render.preview` (the last is the only one that can approach the documented 500 000-character hard limit). The field is additive and defaults to `None`, so catalog descriptors and every existing construction site keep their behaviour; `agent/descriptors/src/version.rs`'s exhaustive-destructuring test now destructures `meta` and asserts catalog data leaves it `None`. The same revision adds the MCP `initialize` result's `instructions` field (`agent/mcp`'s `SERVER_INSTRUCTIONS`), front-loaded so its first 512 characters are self-contained, matching Codex's documented requirement. **E-19 (installation surfaces):** `graphite-agent` gains `print-config`, `install`, and `doctor` (`agent/cli/src/{clients,setup}.rs`). One table in `clients.rs` drives all three plus the generated `agent/INSTALL.md`, which a drift test pins byte-for-byte to `print-config --client all --format markdown`. Two safety rules are enforced by tests: `install` never rewrites a file it cannot round-trip, so Codex (`config.toml`; this workspace has no TOML parser) and Claude Code's `~/.claude.json` (private state and credentials) are configured only through their own `mcp add` commands; and `install` writes only with `--yes`, always keeping a timestamped backup, because a project `.mcp.json` is committed and a careless merge would silently drop another server. `doctor` proves the server works rather than only reading configuration: it starts the real binary and completes an MCP `initialize` handshake. **E-20 (schema shape):** Claude Code flattens a root-level `anyOf`/`oneOf`/`allOf` before sending a tool to the API, which silently weakens the constraint into a description hint, so two guard tests assert that no generated or curated descriptor has a root combinator. None do today — both generators emit object literals — so this is a regression guard rather than a fix.
